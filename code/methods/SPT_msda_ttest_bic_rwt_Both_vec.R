...
SPT_ttest_msda_rwt_bic = function(X1, X2, N1, N2, mu1, mu2, sigma1, sigma2, 
  ...
  if (!(is.null(gammas1_true) || is.null(gammas2_true))){
    ...
    BICs_ora = rep(NA, length(fit_ora$lambda))
    ...
    for (ilambda in 1:length(fit_ora$lambda)){
      ...
      if (sum(w_hat_ora) == -1){
        ...
        BICs_ora[ilambda] = norm(matrix(z_bar_diff_ora, ncol=1) - sigma_hat_ora %*% w_hat_ora,"F")^2 + sum(w_hat_ora == 0) * log(log(n12+n22)) * log(p) / (n12+n22)
      }
    }
    
    lambda.best_ora = fit_ora$lambda[which.min(BICs_ora)]
    lamidx_ora = which(fit_ora$lambda == lambda.best_ora)
    # nonzero_ora = which(BICs_ora != sum(z_bar_diff_ora^2))
    # if (length(nonzero_ora) > 0){
    #   BICs_ora = BICs_ora[nonzero_ora]
    #   lambda.list_sel_ora = fit_ora$lambda[nonzero_ora]
    #   
    #   lambda.best_ora = lambda.list_sel_ora[which.min(BICs_ora)]
    #   lamidx_ora = which(fit_ora$lambda == lambda.best_ora)
    # }else{
  ...
  return(list(p_value = p_value, T_n = T_n, BICs = BICs, lams = fit$lambda,
              p_value_ora = p_value_ora, T_n_ora = T_n_ora, BICs_ora = BICs_ora, lams_ora = fit_ora$lambda,
              w_hat = w_hat, w_hat_ora = w_hat_ora, 
